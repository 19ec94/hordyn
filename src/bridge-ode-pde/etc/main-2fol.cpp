...
void compute_g_Aymard(const Grid& grid, const BioParams& bio_params,
	...
	//double beta = -0.04;
	//double f_i = beta + (s_bar_avg - s_bar_i)/((1.0 - s_bar_i) * x_i);
	double f_i = 0.0;

	for (size_t i = 0; i < grid.edges.size(); ++i) {
		g_edges[i] = term1 + grid.edges[i] * ( - s_bar_i + c1 *  s_bar_avg ) 
				- x_i * grid.edges[i] * f_i  + s_i * f_i;
	}
}
...
void compute_rhs(const Grid& grid,
	...
	for (int i = 0; i < Nf; ++i) {
		...
		compute_p_minus_lambda(bio_params, follicles, i, aymard_params, follicles[i].p_minus_lambda);
	}
...
int main() {
	...
	std::filesystem::create_directories("aymard-2fol");
	std::ofstream case_file("aymard-2fol/case_log.csv");
	if (!case_file) {
		std::cerr << "Error: could not open case_log.csv for writing.\n";
		return 1;
	}
	std::ofstream phi0_file("aymard-2fol/phi0.csv");
	if (!phi0_file) {
		std::cerr << "Error: could not open phi0.csv for writing.\n";
		return 1;
	}
	std::ofstream phi1_file("aymard-2fol/phi1.csv");
	if (!phi1_file) {

...
struct MeshParams {
	...
	int Nm = 10000;       // Number of cells per follicle
};
...
struct Follicle {
	...
	std::vector<double> p_minus_lambda;
	...
	Follicle(const MeshParams& mesh_params)
		: phi_centers(mesh_params.Nm, 0.0)
		  ...
		  , p_minus_lambda(mesh_params.Nm, 0.0)
	{}
};
...
void compute_g_Aymard(const Grid& grid, const BioParams& bio_params,
	...
	const int Nf = follicles.size(); // TODO: Check if the number of follicles is correct!
	...
	double s_bar_avg = s_bar_sum / Nf;

	term1 = (1.0 - c1) * s_bar_avg;

	double s_bar_i = s_i/x_i;


	for (size_t i = 0; i < grid.edges.size(); ++i) {
		g_edges[i] = term1 + grid.edges[i] * (  c1 *  s_bar_avg  ) - grid.edges[i] * grid.edges[i];
	}
}


// Aymard
void compute_p_minus_lambda(const Grid& grid, const BioParams& bio_params,
		const std::vector<Follicle>& follicles,
		int i,
		const std::vector<AymardParams>& aymard_params,
		std::vector<double>& p_minus_lambda) {
	...
	const int Nf = follicles.size(); // TODO: Check if the number of follciles is correct!
	...
	const double lambda = lambda_factor * lambda_sum + 1.0 * s_bar;

	for (size_t i = 0; i < grid.centers.size(); ++i) {
		p_minus_lambda[i] = p - lambda + 1.0*  grid.centers[i];
	}
...
void compute_rhs(const Grid& grid,
	...
	for (int i = 0; i < Nf; ++i) {
		...
		compute_p_minus_lambda(grid, bio_params, follicles, i, aymard_params, follicles[i].p_minus_lambda);
	}
	...
	for (int i = 0; i < Nf; ++i) {
		...
		auto& rhs  = rhs_phi[i];
		auto& pml = follicles[i].p_minus_lambda;
...
void rk4_step(const Grid& grid,
	...
	// k1
	compute_rhs(grid, bio_params, follicles, k1, aymard_params);

	// k2
	for (int i = 0; i < Nf; ++i)
		for (int j = 0; j < Nm; ++j)
			follicles[i].phi_centers[j] = phi_backup[i][j] + 0.5 * dt * k1[i][j];
	compute_rhs(grid, bio_params, follicles, k2, aymard_params);

	// k3
	for (int i = 0; i < Nf; ++i)
		for (int j = 0; j < Nm; ++j)
			follicles[i].phi_centers[j] = phi_backup[i][j] + 0.5 * dt * k2[i][j];
	compute_rhs(grid, bio_params, follicles, k3, aymard_params);

	// k4
	for (int i = 0; i < Nf; ++i)
		for (int j = 0; j < Nm; ++j)
			follicles[i].phi_centers[j] = phi_backup[i][j] + dt * k3[i][j];
	compute_rhs(grid, bio_params, follicles, k4, aymard_params);
...
int main() {
	...
	std::vector<AymardParams> aymard_params = {
		{ .alpha_0=1.2, .c1=0.1, .kappa_0=0.3, .sm=0.9, .sv=0.05, .Um=0.75, .UM=1.5 },
		{ .alpha_0=0.96, .c1=0.1, .kappa_0=0.8, .sm=0.99, .sv =0.05, .Um=0.75, .UM=1.5 }
	};
	...
	for (int i = 0; i < Nf; ++i) {
		compute_p_minus_lambda(grid, bio_params, follicles,
				i, aymard_params, follicles[i].p_minus_lambda);
	}

	TimeParams time_params;
	double t = 0.0;

	std::filesystem::create_directories("aymard-2fol-c1c2");
	std::ofstream case_file("aymard-2fol-c1c2/case_log.csv");
	if (!case_file) {
		std::cerr << "Error: could not open case_log.csv for writing.\n";
		return 1;
	}
	std::ofstream phi0_file("aymard-2fol-c1c2/phi0.csv");
	if (!phi0_file) {
		std::cerr << "Error: could not open phi0.csv for writing.\n";
		return 1;
	}
	std::ofstream phi1_file("aymard-2fol-c1c2/phi1.csv");
	if (!phi1_file) {
		std::cerr << "Error: could not open phi1.csv for writing.\n";
		return 1;
	}

	case_file << "t,dt,max_g,x0,s0,x1,s1\n";
	...
	while (t < time_params.T_end) {
		...
		for (const auto& f : follicles) {
			...
			for (double pmlval : f.p_minus_lambda)
				max_pml = std::max(max_pml, std::fabs(pmlval));
		}
		...
		if (max_pml > 0.0) {
			double dt_react = 0.5 / max_pml; // safety factor 0.5
			dt = std::min(dt, dt_react);
		}

		dt = std::clamp(dt, 1e-6, 1e-2);

		rk4_step(grid, bio_params, follicles, dt, aymard_params);
